document.addEventListener('DOMContentLoaded', function() {
    ...
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
        ...
        // Check if the current tab URL is a Google Maps search page
...
function scrapeData() {
    ...
    return links.map(link => {
        ...
        // Phone number extraction with improved regex
        if (container) {
            var containerText = container.textContent || '';
            var phoneRegex = /(?:\+?\d{1,3})?[-.\s]?(?:\(\d{1,4}\)|\d{1,4})[-.\s]?\d{1,4}[-.\s]?\d{1,4}(?:[-.\s]?\d{1,9})?/g;
            var phoneMatches = containerText.match(phoneRegex);
            if (phoneMatches) {
                phoneMatches = phoneMatches.filter(match => {
                    var digitsOnly = match.replace(/\D/g, '');
                    var isValidPhone = digitsOnly.length >= 7 && digitsOnly.length <= 15;
                    return isValidPhone;
                });
                phone = phoneMatches.join(', ').trim();
            } else {
                phone = '';
            }
        }
